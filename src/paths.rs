...
pub(crate) fn profile_dir_for_read() -> io::Result<PathBuf> {
    profile_dir_for_read_from_env(|key| env::var_os(key))
}

pub(crate) fn fabric_config_path_for_read() -> io::Result<PathBuf> {
    fabric_config_path_for_read_from_env(|key| env::var_os(key))
}
...
{
    ensure_witness_migrated_from_env(get_env)?;
    Ok(witness_ledger_path_from_env(get_env))
}

fn profile_dir_for_read_from_env<F>(get_env: F) -> io::Result<PathBuf>
where
    F: Fn(&str) -> Option<OsString> + Copy,
{
...
fn fabric_config_path_for_read_from_env<F>(get_env: F) -> io::Result<PathBuf>
where
    F: Fn(&str) -> Option<OsString> + Copy,
{
    let canonical = fabric_config_path_from_env(get_env);
    migrate_file_from_env(
        get_env,
        "profile_config",
        &canonical,
        legacy_fabric_config_paths_from_env(get_env),
    )?;
    Ok(canonical)
}

fn fabric_config_path_from_env<F>(get_env: F) -> PathBuf
where
    F: Fn(&str) -> Option<OsString> + Copy,
{
    cmdrvl_root_from_env(get_env)
...
fn legacy_fabric_config_paths_from_env<F>(get_env: F) -> Vec<PathBuf>
where
    F: Fn(&str) -> Option<OsString> + Copy,
{
    let mut paths = Vec::new();
    if let Some(home) =
        non_empty_env(get_env, "HOME").or_else(|| non_empty_env(get_env, "USERPROFILE"))
    {
        paths.push(PathBuf::from(home).join(".epistemic").join("config.toml"));
    }
    paths.push(PathBuf::from(".epistemic").join("config.toml"));
    paths
}
    ...
    for entry in fs::read_dir(source)? {
        ...
        if file_type.is_dir() {
            ...
            return Err(io::Error::new(
                ...
                "unsupported non-regular entry in legacy profile config",
            ));
        }
    }
...
mod tests {
    use super::{
        fabric_config_path_for_read_from_env, profile_dir_for_read_from_env,
        witness_ledger_path_for_append_from_env, witness_ledger_path_from_env,
    };
    ...
    fn witness_append_migrates_legacy_ledger() {
        let tmp = tempfile_dir("profile-paths-witness");
        let home = tmp.join("home");
        let legacy = home.join(".epistemic").join("witness.jsonl");
        fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        fs::write(&legacy, "{\"tool\":\"profile\",\"outcome\":\"OLD\"}\n").unwrap();
    ...
    fn profile_read_migrates_legacy_profile_dir() {
        let tmp = tempfile_dir("profile-paths-profiles");
        let home = tmp.join("home");
    ...
    #[test]
    fn fabric_config_read_migrates_legacy_config() {
        let tmp = tempfile_dir("profile-paths-config");
        let home = tmp.join("home");
        let legacy = home.join(".epistemic").join("config.toml");
        fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        fs::write(&legacy, "[fabric]\nurl = \"http://localhost:9000\"\n").unwrap();

        let canonical = fabric_config_path_for_read_from_env(env_for_home(&home)).unwrap();

        assert_eq!(canonical, home.join(".cmdrvl/config/profile/config.toml"));
        assert!(
        ...
        assert!(
            fs::read_to_string(home.join(".cmdrvl/notices/deprecated-paths.jsonl"))
                .unwrap()
                .contains("\"path_class\":\"profile_config\"")
        );
