...
Commands:
  ...
  push <FILE>            Publish a frozen profile to data-fabric
  pull <PROFILE_ID>      Fetch a frozen profile by ID from data-fabric
...
profile draft new --format <FORMAT> --out <FILE>
  --format <FORMAT>      csv | pdf | xlsx | parquet | jsonl

profile draft init <DATASET> --out <FILE> [--format <FORMAT>] [--key <COLUMN>]
  --format <FORMAT>      csv (default, v0.1); others reserved
...
profile list [--json]
  (searches built-ins, ~/.epistemic/profiles/, EPISTEMIC_PROFILE_PATH)
...
profile push <FROZEN_PROFILE>
  (publishes to data-fabric; thin HTTP wrapper)

profile pull <PROFILE_ID> --out <DIR>
  (fetches from data-fabric; thin HTTP wrapper)
```

### Common flags (all subcommands)

- `--describe`: Print `operator.json` to stdout and exit 0. Checked before input is validated.
- `--schema`: Print JSON Schema for profile YAML to stdout and exit 0. Like `--describe`, checked before input is validated.
...
Network subcommands (`push`/`pull`) return `0` on success and `2` on refusal/transport failure (no domain-level `1` outcome).
...
| `format` | string | yes | `csv`, `xlsx`, `pdf`, `parquet`, `jsonl` |
...
- `profile_version` = integer, monotonic within family
...
2. `--profile-id <ID>` resolved via (first match wins):
   a. Built-in profiles (bundled with the `profile` CLI from `epistemic/profiles/`)
   b. `~/.epistemic/profiles/`
   c. `EPISTEMIC_PROFILE_PATH` env var (colon-separated directories, searched left to right)

```bash
# List available profiles
profile list
# csv.v0 (built-in), csv.loan_tape.core.v0 (~/.epistemic/profiles/), ...
```
...
| `E_BAD_VERSION` | Version is negative, or `--family` / `--version` fails format validation | Fix version or family string |
...
| `E_EMPTY` | Dataset missing header or has no rows | Provide a non-empty dataset |
...
E_BAD_VERSION:
  { "family": "csv..bad name", "version": 0, "error": "family must be dot-separated alphanumeric segments" }
...
E_EMPTY:
  { "path": "tape.csv", "reason": "no header row" }
```
...
3. Sets `key` to the provided `--key`, to the top `suggest-key` candidate when `--key auto`, or to an empty list otherwise
...
```
 ...
 5. Dispatch to subcommand handler:
    ...
    lint:
      ...
      c. Parse dataset header              → E_CSV_PARSE if invalid
      ...
      f. Exit 0 (all clear) or 1 (issues found) or 2 (refusal from step a only)

    stats:
      a. Open dataset file                 → E_IO if not found or permission denied
      b. Parse dataset                     → E_CSV_PARSE if invalid
    ...
    freeze:
      ...
      e. Validate --family format and --version ≥ 0 → E_BAD_VERSION if invalid
    ...
    list:
      a. Search resolution paths (built-in, ~/.epistemic/profiles/, EPISTEMIC_PROFILE_PATH)
...
pub enum Command {
    ...
    /// Publish a frozen profile to data-fabric
    Push {
        /// Frozen profile YAML
        file: PathBuf,
    },
    /// Fetch a frozen profile by ID from data-fabric
    Pull {
        /// Profile ID
        profile_id: String,
        /// Output directory
...
pub enum DraftAction {
    ...
    New {
        /// Format (csv, xlsx, pdf, parquet, jsonl)
        #[arg(long)]
        format: String,
        /// Output path
        #[arg(long)]
...
```json
{
  ...
  "subcommands": [
    ...
    { "name": "push", "description": "Publish frozen profile to data-fabric" },
    { "name": "pull", "description": "Fetch frozen profile from data-fabric" }
  ],
  ...
  "refusals": [
    ...
    { "code": "E_BAD_VERSION", "message": "Invalid version or family format", "action": "fix_input" },
    ...
    { "code": "E_EMPTY", "message": "Dataset missing header or empty", "action": "fix_input" },
...
- **Draft new tests:** blank template generation for each format
...
- **List tests:** finds built-in and user profiles
...
- `profile list` (search resolution paths)
